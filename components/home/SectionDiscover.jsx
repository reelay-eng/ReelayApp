...
const SectionDiscover = ({ navigation, route, refreshControl }) => {
  try {
    ...
    const FilterButton = React.memo(({ filterKey }) => {
    ...
    const WatchlistFilters = React.memo(() => {
      const filterKeys = ["Newest", "Following", "Watchlist", "More Filters"];
      return (
        <FilterRowView>
          {filterKeys.map((key) => (
    ...
    const loadFeed = React.useCallback(
      async (items) => {
        ...
        if (feedSource === "discover") {
          ...
          if (sortMethod === "thisMonth")
            dispatchAction = "setDiscoverThisMonth";
    ...
    React.useEffect(() => {
      loadFeed(selectedSection);
    }, [selectedSection, loadFeed]);

    const extendFeed = React.useCallback(
      async (items) => {
