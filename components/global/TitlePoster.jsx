...
const TitlePoster = ({ onLoad = () => {}, title, onPress, width = 60 }) => {
  ...
  const getTitlePosterSource = () => {
    if (title?.posterSource === PLACEHOLDER_POSTER_SOURCE)
      return PLACEHOLDER_POSTER_SOURCE;
    if (title?.posterSource === WELCOME_VIDEO_POSTER_SOURCE)
      return WELCOME_VIDEO_POSTER_SOURCE;
    if (loadState === "local") {
    ...
    <SafeAreaView>
      <PosterContainer
        onPress={() => {
          if (userCanPress) {
            if (onPress) onPress();
            setUserCanPress(false);
            setTimeout(() => {
              setUserCanPress(true);
            }, 1000);
          }
        }}
      >
